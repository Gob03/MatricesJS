...
/*------------------------------------------------------
					FUNCIONES COMUNES
--------------------------------------------------------*/
//  AQUI createTablesInputs crea  1 inputs y una tabla
...
// MODIFICADO
	const checkInputsOrdenIsNotEmpty = () => {
		...
		if (validityCounter == ordenInputs.length){
			...
			createTablesInputs(IDtablesInputsContainer)
			createBtn({
	...
	const createTablesInputs = () => {
		...
		if( posBtn.getAttribute("valuePos") == "before"){
			let cantInputs = 0
	...
	const createResultTable = () => {
		...
		for(let i = 0; i < table0rowsColums[0]; i++){
			...
			for(let j = 0; j < table0rowsColums[1]; j++){
				...
				if(posBtn.getAttribute("valuePos") == "before"){
					span.innerHTML = parseFloat(inputValor) / parseFloat(table0InputsArray[cont].value)
				}
	...
	const changeValuePosition = () => {
		...
		if(posBtn.getAttribute("valuePos") == "before"){
			posBtn.setAttribute("valuePos", "after")
			posBtn.value = "Valor: Despues"
		}

		else{
			posBtn.setAttribute("valuePos", "before")
			posBtn.value = "Valor: Antes"
		}
	...
	const posBtn = document.getElementById("posBtn")
	posBtn.addEventListener("click", changeValuePosition)
