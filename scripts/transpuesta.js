...
/*------------------------------------------------------
					FUNCIONES COMUNES
--------------------------------------------------------*/
//  AQUI createTablesInputs crea 1 Tabla
	...
	const createResultTable = () => {
		...
		const tablesRowsColums = searchingRowsAndColumns(table0)
		...
		for(let i = 0; i < tablesRowsColums[1]; i++){
			let row = document.createElement("tr")
			table.appendChild(row)

			for(let c = 0, f = i; c < tablesRowsColums[0]; c++, f += tablesRowsColums[1]){
				let column = document.createElement("td")
				row.appendChild(column)

				let span = document.createElement("span")
				span.innerHTML = table0[f].value
